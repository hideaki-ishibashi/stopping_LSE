...
if __name__ == '__main__':
    func_name_list = ["rosenbrock", "booth", "sphere", "branin"]
    true_noise_sigma = [{"rosenbrock": 30, "booth": 30, "sphere": 2, "branin": 20}]
    budget_list = [{"rosenbrock": 150, "booth": 200, "sphere": 300, "branin": 500},
                   {"rosenbrock": 200, "booth": 200, "sphere": 300, "branin": 500}]
    th_list = [{"rosenbrock": 500, "booth": 300, "sphere": 30, "branin": 50},
               {"rosenbrock": 300, "booth": 100, "sphere": 10, "branin": 75}]
    ...
    save_dir = "result/test_func/change_threshold"
    os.makedirs(save_dir, exist_ok=True)

    # plot true upper/lower level set
    plt.figure(0, [7, 7])
    for t, func_name in enumerate(func_name_list):
        for s in range(len(th_list)):
            # set test function
            plt.clf()
            target = TestFunction(function_name=func_name, threshold=th_list[s][func_name], noise_std=true_noise_sigma[0][func_name], resolution=100)
            env = target.env_name
            X = target.candidate_points
            y = target.function(target.candidate_points)
            c = np.where(y >= target.threshold, 1, 0)
            grid = X.reshape(100, 100, 2)
            y_grid = y.reshape(100, 100)
            c_grid = c.reshape(100, 100)
            plt.contourf(grid[:, :, 0], grid[:, :, 1], c_grid, cmap="bwr")
            plt.savefig(f"{save_dir}/{env}_true_upper_lower_set.pdf", bbox_inches="tight")
    ...
    for s in range(len(th_list)):
        for t, func_name in enumerate(func_name_list):
            for n in range(len(true_noise_sigma)):
                # set test function
                target = TestFunction(function_name=func_name, threshold=th_list[s][func_name], noise_std=true_noise_sigma[n][func_name], resolution=resolution)
                env = target.env_name
                setting_list[env] = {}
                for a, acq_func_name in enumerate(acq_func_list):
                    params = set_params(acq_func_name, L, delta, None, beta, nu, batch_size_dict[acq_func_name],
                                        num_iters, step_size)
                    setting_list[env][acq_func_name] = ExpSetting(target, acq_func_name, rule_name_dict[acq_func_name],
                                                                  budget_list[s][func_name],
                                                                  batch_size_dict[acq_func_name], n_iteration,
                                                                  stopping_criteria, params)
    ...
    for e, env in enumerate(setting_list.keys()):
        for a, acq_func_name in enumerate(setting_list[env].keys()):
            ...
            for i in range(n_iteration):
                ...
                for s, sc in enumerate(lse.stopping_criteria):
                    ...
                    print(f"{sc.name}:{setting.stopping_time[s, i]}:{setting.stopping_score[s, i]}")
        ...
        for a, acq_func_name in enumerate(acq_func_list):
            ...
            for s in range(len(stopping_criteria)):
                ...
                if s == 0:
                    ...
                    plt.scatter(st, ss, c=color_list[a], marker="^", label=f"FC({acq_func_name})", zorder=len(acq_func_list) - a,
        ...
        plt.savefig(f"{save_dir}/{env}_f1_and_st.pdf", bbox_inches="tight")
        ...
        for a, acq_func_name in enumerate(acq_func_list):
            ...
            for s in range(len(stopping_criteria)):
                st = setting.stopping_time[s]
                ss = setting.stopping_score[s]
                outline = mpe.withStroke(linewidth=4, foreground=color_list[a], capstyle="projecting")
                if s == 0:
                    if a == 0:
                        plt.errorbar(st.mean(), ss.mean(), xerr=st.std(), yerr=ss.std(),
                                     c="white", zorder=5 * len(acq_func_list) - a, capsize=0, lw=1,
                ...
                if s == 1:
                    ...
                    plt.errorbar(st.mean(), ss.mean(), xerr=st.std(), yerr=ss.std(),
                                 c="white", zorder=5 * len(acq_func_list) - a, capsize=0, lw=1, path_effects=[outline])
                    if a == 0:
                        plt.scatter(st, ss, c=color_list[a], marker="^", label=f"FC({acq_func_name})", zorder=2 * len(acq_func_list) - a, s=130, alpha=st_alpha, edgecolors="k")
                    else:
                        plt.scatter(st, ss, c=color_list[a], marker="^", zorder=2*len(acq_func_list)-a, s=130, alpha=st_alpha, edgecolors="k")
        ...
        plt.savefig(f"{save_dir}/{env}_st.pdf", bbox_inches="tight")
        ...
        if setting.target.function_name == "sphere" and np.isclose(setting.target.noise_std, 0.0):
            ...
            legend.set_zorder(4*len(acq_func_list))
        plt.savefig(f"{save_dir}/{env}_f1.pdf", bbox_inches="tight")
        ...
        for a, acq_func_name in enumerate(acq_func_list):
            ...
            for s in range(len(stopping_criteria)):
                ...
                if s == 0:
                    if a == 0:
                        outline = mpe.withStroke(linewidth=4, foreground="k", capstyle="projecting")
                        plt.errorbar(st.mean(), ss.mean(), xerr=st.std(), yerr=ss.std(),
                                     c="white", label="Ours", zorder=5, capsize=0, lw=1, path_effects=[outline])
                else:
                    plt.errorbar(st.mean(), ss.mean(), xerr=st.std(), yerr=ss.std(),
                                 c="white", label=f"FC({acq_func_name})", zorder=4, capsize=0, lw=1, path_effects=[outline])
        for a, acq_func_name in enumerate(acq_func_list):
            setting = setting_list[env][acq_func_name]
            plt.plot(setting.sample_span, setting.f1_score.mean(axis=0), c=color_list[a], label=f"{acq_func_name}", lw=2, alpha=1.0)
            plt.fill_between(setting.sample_span, setting.f1_score.mean(axis=0)-setting.f1_score.std(axis=0), setting.f1_score.mean(axis=0)+setting.f1_score.std(axis=0), color=color_list[a], lw=2, alpha=0.2)

        if setting.target.function_name == "sphere" and np.isclose(setting.target.noise_std, 0.0):
            legend = plt.legend(borderaxespad=0, ncol=2, handlelength=0.5, fontsize=24, loc="lower right")
            legend.set_zorder(4*len(acq_func_list))
        plt.savefig(f"{save_dir}/{env}_f1_error_and_st_error.pdf", bbox_inches="tight")
